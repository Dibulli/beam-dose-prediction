                ...
                for dicom_rtst_series in pred_dicomdir.series_iter(series_type='RT Structure Set Storage'):
                    ...
                    with torch.no_grad():
                        for z in range(z_dim):
                            ...
                            # masks = einops.rearrange(masks, 'h w (b c) -> b c h w', b=1)

                            # body_slice = einops.repeat(body_slice, "(b c h) w -> (repeat b) c h w", repeat=9, b=1, c=1)
                            # input_ib = np.concatenate((img, contours, masks), axis=1)
                    ...
                    np.save(output_folder + os.sep + str(pt_id) + "_-1.npy", total_pred)
