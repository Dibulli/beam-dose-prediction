...
# csv_file = pd.read_csv("/data/szh/h5_512_prune_ptv/test_20_pts/data_recoder.csv", dtype=str, header=None)
# h5_list = csv_file.iloc[:, 0]
# pt_id = csv_file.iloc[:, 1]
# min_range = csv_file.iloc[:, 3]
# max_range = csv_file.iloc[:, 4]

class Total_Dice_Calculator(cfg):
    def __int__(self, input_real, input_pred, thresh):
...
dir_path = "/Volumes/NPC预测数据盘/单野预测2024/pred_dose/dvh_and_dice"

with open("/data/szh/dvh_calculation/real.csv", 'w+') as csv_f:
...
for index in range(len(csv_file)):
    print(pt_id[index])
    h5_name = "/data/szh/h5_512_prune_ptv/test_20_pts/" + h5_list[index] + '.h5'
    f = h5.File(h5_name, 'r')
    # pred = np.load("/data/szh/dose_output/transformer/" + pt_id[index] + "_-1.npy")
    # pred_sum = einops.rearrange(pred, 'b c h w z -> (b c) h w z')
    # real_dose = pred_sum[0, :, :, int(min_range[index]):int(max_range[index])]

    mask_npy = f["slice_mask"]
    ...
    with open("/data/szh/dvh_calculation/real.csv", 'a+') as csv_file:
        csv_file.write("{},{},{},{},{},"
                       "{},{},{},{},{},"
                       "{},{},{},{},{},"
                       "{},{},{},{},"
